...
Original file is located at
    https://colab.research.google.com/github/Artur4ik2304/Yandex_ML_3.0/blob/main/hw_07_nn_from_scratch/modules.ipynb
...
class Sequential(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = input
        for elem in self.module:
          self.output = elem.forward(self.output)
        return self.output
    ...
    def backward(self, input, gradOutput):
        ...
        for i in range(len(self.modules) - 2, 0, -1):
          self.gradInput = self.modules[i + 1].backward(self.modules[i].ouput,
                                                        self.gradInput)
...
class Linear(Module):
    ...
    def updateOutput(self, input):
        self.output = np.add(np.matmul(self.W, input), self.b)
        return self.output
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.divide(np.exp(self.output), np.sum(np.exp(self.output), axis=1))
        return self.output
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
      # Нормализация по строкам (axis=1)
      input_max = np.max(input, axis=1, keepdims=True)
      self.output = input - input_max

      # Логарифм суммы экспонент (log-sum-exp)
      log_sum_exp = np.log(np.sum(np.exp(self.output), axis=1, keepdims=True))

      # LogSoftMax = input - max - log(sum(exp(input - max)))
      self.output -= log_sum_exp

      return self.output

    def updateGradInput(self, input, gradOutput):
      # Градиент LogSoftMax: gradOutput - sum(gradOutput) * softmax
      softmax = np.exp(self.output)  # Так как self.output = log(softmax)
      self.gradInput = gradOutput - np.sum(gradOutput, axis=1, keepdims=True) * softmax
      return self.gradInput
...
class BatchNormalization(Module):
    ...
    def __init__(self, alpha=0.):
        ...
        self.alpha = alpha  # momentum for moving average
        self.moving_mean = 0.0
        self.moving_variance = 1.0  # Initialize to 1 to avoid division by zero
        self.initialized = False

    def updateOutput(self, input):
        if input.ndim != 2:
            raise ValueError("Expected 2D input (batch_size, features)")

        if not self.training:
            # Inference mode - use moving statistics
            if not self.initialized:
                raise RuntimeError("BatchNorm: must be trained before inference")
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        dmean = np.sum(gradOutput * -std_inv, axis=0) + dvar * np.mean(-2 * x_centered, axis=0)
...
class Dropout(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        if not self.training:
            ...
            # Use the same mask as in forward pass
...
class ELU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.multiply(gradOutput, input > 0) + np.multiply(gradOutput, (input <= 0) * self.slope * np.exp(input))
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target):
        ...
        log_probs = np.log(input_clamp) * target
        sample_losses = -np.sum(log_probs, axis=1)
...
class ClassNLLCriterion(Criterion):
    ...
    def updateGradInput(self, input, target):
        ...
        self.gradInput = (softmax - target) / input.shape[0]
        return self.gradInput
...
def adam_optimizer(variables, gradients, config, state):
    ...
    for k in ['learning_rate', 'beta1', 'beta2', 'epsilon']:
        assert k in config, f"Missing config parameter: {k}"
